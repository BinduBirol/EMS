/**
 * EMS Demo Menu ------------------ You should not use this file in production.
...
var this_page_id;
		...
		$(".nav-link").click(function() {
			if (!this.id == "") {
				getPageData(this.id);
				$(".nav-toogler").removeClass('active');				
			}
		});
	
		function getPageData(id) {	
			$.ajax({
				url : 'getNavPage?navid=' + id,
				type : 'GET',
				cache: false,
				success : function(data) {
					this_page_id=id;
					$('#page_content_ajax').html("");
					//$("#page_content_ajax").hide();
					$('#page_content_ajax').html(data);
					$("#page_content_ajax").fadeIn("slow");
					document.title = id;
					$(".nav-link").removeClass("active");
					$("#"+id).addClass("active");
					$("#"+id ).closest( ".nav-toogler" ).addClass('active');
					i++;
				},
				 error: function (jqXHR, exception) {
					 ...
					 $('#page_content_ajax').html("");
						//$("#page_content_ajax").hide();
						$('#page_content_ajax').html("<div class='content-wrapper bg-danger'><section class='content '>"+jqXHR.responseText+"</section></div>");
						$("#page_content_ajax").fadeIn("slow");
...
(function ($) {
  ...
  setTimeout(function () {
    if (window.___browserSync___ === undefined && Number(localStorage.getItem('NetLit:Demo:MessageShowed')) < Date.now()) {
      localStorage.setItem('NetLit:Demo:MessageShowed', (Date.now()) + (15 * 60 * 1000))
      // eslint-disable-next-line no-alert
      // alert('You load NetLit\'s "demo.js", \nthis file is only created for
		// testing purposes!')
    }
  }, 1000)
  ...
  $container.append(
    '<h5>Customize NetLit</h5><hr class="mb-2"/>'
  )
...
*/
  ...
  // $accent_variants.find('option.' + active_accent_color).prop('selected',
	// true)
  // $accent_variants.removeClass().addClass('custom-select mb-3 text-light
	// border-0 ').addClass(active_accent_color)
